...
第三阶段保持单写入连接，新增一个只读连接执行完整对账，使用显式读事务保持跨查询快照一致。版本6的 `daily_game_totals` 从原始游戏流水回填，通过SQLite INSERT触发器在原事务中维护；不是独立异步缓存，也不能代替原始账本。离线检查器会核对日汇总、逐笔流水余额和账本/冻结，并且不运行Service初始化或发送恢复。规模结果、升级/回退及恢复验证见 [第三阶段记录](DATA_PHASE3.md)。
