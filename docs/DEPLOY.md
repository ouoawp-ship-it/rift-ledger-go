...
先以 **不启动真实Telegram的隔离验证环境** 检查恢复库的结果、账本与待发送消息，避免回滚offset/outbox带来消息重发；确认后再恢复正式Token。第三阶段新增 `bash scripts/verify-backup.sh backups/实际文件.db`，使用镜像内离线工具、禁用网络、仅挂载只读备份，不覆盖正式卷。合成数据完整恢复演练与真实备份独立副本的只读核验已通过，具体范围见 [第三阶段记录](DATA_PHASE3.md)。生产恢复仍需人工核实备份时点和后续业务，不提供破坏性一键覆盖脚本。

## 运行限制

仅Linux，CGO_ENABLED=1，单SQLite业务连接串行事务/查询，完整对账使用独立只读快照连接。本版追求清楚的事务边界与可复测，不声称无限并发；磁盘满、断电和长时间运行仍需验收。不要多副本部署、不要使用NFS或复制Token启动第二接收器。
