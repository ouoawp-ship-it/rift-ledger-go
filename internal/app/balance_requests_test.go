...
func TestParseBalanceRequest(t *testing.T) {
	...
	for _, text := range valid {
		...
		if !ok || amount != Points(100) || (kind != "CREDIT" && kind != "DEBIT") {
			t.Fatalf("%q => %q %d %v", text, kind, amount, ok)
		}
	}
	for _, text := range []string{"上分0", "下分-1", "回1.1234", "上分1000000000001", "上分1e2", "上分abc", "上分"} {
...
func TestApprovedTopUpAutomaticallyEnablesPlayer(t *testing.T) {
	...
	a := acc(t, s, "tg:333")
	if !a.Enabled || a.Balance != Points(100) {
