...
func composeHeroImage(raw [][]byte, photos []app.MediaPhoto) ([]byte, error) {
	if len(raw) != 5 || len(photos) != 5 {
		return nil, errors.New("英雄图片数量无效")
	}
	const tile = 320
	...
	for i, b := range raw {
		...
		badge := color.RGBA{40, 116, 220, 235}
		label := "闲"
		if banker {
			badge = color.RGBA{210, 45, 45, 240}
			label = "庄"
		}
		...
		if face == basicfont.Face7x13 {
			d.DrawString(fmt.Sprintf("%d %s", i+1, label))
		} else {
	...
	for _, path := range paths {
		...
		if collection, err := opentype.ParseCollection(b); err == nil {
			if f, err := collection.Font(0); err == nil {
				if face, err := opentype.NewFace(f, &opentype.FaceOptions{Size: 34, DPI: 72, Hinting: font.HintingFull}); err == nil {
					return face
				}
			}
		}
