...
(() => {
 ...
 const senderLabels={idle:'暂无待发任务',working:'正在发送',delayed:'等待时间较长',rate_limited:'限流等待，稍后自动继续',blocked:'需要处理'};
 ...
 async function loadOperations(){
  ...
  try{
   ...
   healthCard('ops-sender',q.state,senderLabels[q.state]||'未知',q.pending+' 条待发 · '+q.inflight+' 条发送中 · '+q.needs_review+' 条待处理'+(q.retry_at?' · 预计 '+when(q.retry_at)+' 后继续':''));
