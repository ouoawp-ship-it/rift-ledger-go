...
bindForm('rules-form',async()=>{const r={payout:Array.from({length:11},(_,i)=>Number($('odd-'+i).value)),min_stake:$('min-stake').value.trim(),max_stake:$('max-stake').value.trim(),zero_triple:$('zero-triple').value==='true',confirmed:true};const saved=await api('rules',{expected_version:rulesVersion,rules:r},true);rulesVersion=saved.version;await refreshState();renderRules();notice('规则设置已保存，仅影响后续开盘期次。玩家模式已启用。');});
