...
bindForm('rules-form',async()=>{const r={payout:Array.from({length:11},(_,i)=>Number($('odd-'+i).value)),min_stake:Number($('min-stake').value),max_stake:Number($('max-stake').value),zero_triple:$('zero-triple').value==='true',confirmed:true};await api('rules',{expected_version:rulesVersion,rules:r},true);await refreshState();renderRules();notice('规则设置已保存，仅影响后续开盘期次。玩家模式已启用。');});
...
bind('requests-refresh',loadRequests);bind('request-alert-close',hideRequestAlert);bind('request-alert-dismiss',hideRequestAlert);bind('request-alert-view',async()=>{hideRequestAlert();await showTab('requests');});bind('champions-refresh',async()=>{await api('champions/refresh',{},true);await loadChampions();notice('英雄数据已刷新；失败时原缓存会保留。')});bind('bot-test',async()=>{const x=await api('bot-settings/test',{},true);notice('连接成功：@'+x.username)});bind('bot-test-group',async()=>{await api('bot-settings/test-group',{},true);notice('群测试消息已加入发送队列')});bindForm('bot-form',async()=>{await api('bot-settings',{token:$('bot-token').value,bot_username:$('bot-username').value,group_id:Number($('bot-group').value||0),admin_id:Number($('bot-admin').value||0),support_username:$('bot-support').value,enabled:$('bot-enabled').checked,revision:Number(($('bot-form').dataset.revision||0))},true);notice('配置已保存，服务正在自动重启并应用新设置，请稍候。');setTimeout(()=>location.reload(),1200)});
