...
async function loadPlayers(){const page=offsets.players,d=await api('accounts?limit=50&offset='+page),rows=d.rows||d,stats=d.stats||{};$('players-page').textContent='第'+(page/50+1)+'页（每页最多50条）';$('players-table').innerHTML='<p class="summary">玩家数量：'+Number(stats.players||0).toLocaleString('zh-CN')+'｜玩家总分：'+fmt(stats.balance||0)+'｜今日盈亏：'+signed(stats.profit||0)+'</p>'+table(['玩家TG ID','玩家昵称','玩家TG用户名','分数','本期下注金额','下注内容','上期盈亏','操作'],rows.map(a=>[esc(a.telegram_id),esc(a.name),esc(a.username?'@'+a.username:'未设置'),tdnum(a.balance),tdnum(a.round_stake||0),esc(a.round_content||'—'),signed(a.last_profit||0),(a.enabled?'<button data-disable-player="'+esc(a.telegram_id)+'" data-name="'+esc(a.name)+'">停用玩家</button>':'<span class="hint">等待上分审批自动开通</span>')+' <button data-account="'+esc(a.id)+'">调分</button> <button data-player-history="'+esc(a.id)+'">历史明细</button>']));}
