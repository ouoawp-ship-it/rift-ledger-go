...
bindForm('login-form',async()=>{unlockAlertAudio();token=$('token').value.trim();try{await refreshState(true);}catch(e){token='';throw e;}$('token').value='';$('login-box').hidden=true;$('workspace').hidden=false;await showTab('round');startRequestWatcher();startBotConnectionWatcher();notice('已连接。首次运行请先刷新英雄数据并确认赔率。');});
bind('logout',async()=>{stopBotConnectionWatcher();if(requestPollTimer)clearInterval(requestPollTimer);token='';state=null;previewData=null;pending.clear();location.reload();});
