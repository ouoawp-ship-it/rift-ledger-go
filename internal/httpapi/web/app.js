...
function renderRules(){const r=state.rules;rulesVersion=state.rules_version;$('rules-version').textContent='后端模板版本：'+rulesVersion;
 ...
 $('fee-timing').value=r.confirmed?r.fee_timing:'';$('void-fee').value=r.confirmed?r.void_fee:'';$('fee-recipient').value=r.confirmed?r.fee_recipient:'';$('zero-triple').value=r.confirmed?String(r.zero_triple):'';$('rules-confirmed').checked=r.confirmed;
}
...
function renderPreview(p){return '<div class="summary"><strong>'+esc(p.number)+'期</strong>｜'+(p.whole_void?'整期流局：'+esc(p.reason):'正常结算')+'｜实际时长'+p.duration_seconds+'秒<br>玩家游戏合计 '+signed(p.player_game_delta)+'；运营方游戏合计 '+signed(p.house_game_delta)+'；最终费用 '+fmt(p.fee_total)+'；注单 '+p.lines.length+'笔</div><div class="table-wrap">'+positionTable(p)+'</div><h4>逐笔结算'+(p.lines.length>200?'（仅展示前200笔，汇总涵盖全部）':'')+'</h4><div class="table-wrap">'+table(['玩家账户','位置','本金','结果','游戏变化','最终费用','全生命周期净变化'],p.lines.slice(0,200).map(l=>[esc(l.account_id),esc(l.position),tdnum(l.stake),esc(states[l.outcome]),esc(signed(l.game_delta)),tdnum(l.fee),esc(signed(l.net_delta))]))+'</div>';}
...
async function loadPlayers(){const page=offsets.players,rows=await api('accounts?limit=50&offset='+page);$('players-page').textContent='第'+(page/50+1)+'页（每页最多50条）';$('players-table').innerHTML=table(['称呼','账户','类型','允许下注','余额','冻结','可用','操作'],rows.map(a=>[esc(a.name),esc(a.id),esc({player:'玩家',house:'运营方',fees:'费用账户'}[a.role]),a.role==='player'?(a.enabled?'是':'否'):'—',tdnum(a.balance),tdnum(a.locked),tdnum(a.available),'<button data-account="'+esc(a.id)+'">选择调分</button>'+(a.role==='player'?' <button data-player="'+esc(a.telegram_id)+'" data-name="'+esc(a.name)+'" data-enabled="'+a.enabled+'">编辑资格</button>':'')]));}
...
bindForm('login-form',async()=>{token=$('token').value.trim();try{await refreshState(true);}catch(e){token='';throw e;}$('token').value='';$('login-box').hidden=true;$('workspace').hidden=false;await showTab('round');notice('已连接。首次运行请先确认规则、开通玩家，并为玩家与运营方登记积分。');});
...
bind('choose-house',async()=>{$('adjust-id').value='house';});bind('choose-fees',async()=>{$('adjust-id').value='fees';});
...
bindForm('rules-form',async()=>{const r={payout:Array.from({length:11},(_,i)=>Number($('odd-'+i).value)),min_stake:Number($('min-stake').value),max_stake:Number($('max-stake').value),fee_timing:nonempty($('fee-timing').value,'请选择费用扣取时点'),void_fee:nonempty($('void-fee').value,'请选择流局费用处理'),fee_recipient:nonempty($('fee-recipient').value,'请选择费用归属'),zero_triple:nonempty($('zero-triple').value,'请选择零组合口径')==='true',confirmed:$('rules-confirmed').checked};await api('rules',{expected_version:rulesVersion,rules:r},true);await refreshState();renderRules();notice('规则模板已保存，仅影响后续开盘期次。'+(r.confirmed?'':'尚未确认，不能开始受理。'));});
